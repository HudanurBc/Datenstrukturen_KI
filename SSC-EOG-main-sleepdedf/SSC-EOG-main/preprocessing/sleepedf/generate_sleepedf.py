...
def gen_sleepedf(files:np.ndarray,save_path:str):
    ...
    ######## Pretext files########
    num_pretext = min(len(files), 58)
    pretext_files = list(np.random.choice(files, num_pretext, replace=False))
    ...
    # Binäres Mapping: REM (Klasse 4) -> 1, alle anderen Klassen -> 0
    binary_y = np.where(y == 4, 1, 0)

    data_save = dict()
    data_save["samples"] = torch.from_numpy(X.transpose(0, 2, 1))
    data_save["labels"] = torch.from_numpy(binary_y)

    torch.save(data_save, os.path.join(save_path, "pretext.pt"))

    ######## Training files ##########
    remaining_files = sorted(list(set(files) - set(pretext_files)))
    num_train = min(len(remaining_files), 10)
    if num_train > 0:
        training_files = list(np.random.choice(remaining_files, num_train, replace=False))
    else:
        training_files = []
